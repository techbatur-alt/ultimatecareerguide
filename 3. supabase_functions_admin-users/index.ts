...
// Requires caller to be staff (service role and above)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create_user") {
      ...
      // Only executive can create executive
      ...
      // Try invite-by-email first (requires SMTP). Fall back to direct create.
      ...
      if (newId) {
        // Ensure profile row exists with desired role (handle_new_user trigger creates one)
      ...
      return json({
        ...
        message: inviteSent
          ? "Invite email sent."
          : "User created. Email delivery isn't configured — share the temporary password securely with the user.",
      });
    }
    ...
    if (action === "reset_password") {
      ...
      const redirectTo = `${req.headers.get("origin") ?? ""}/create-password`;
      const { error: linkErr } = await admin.auth.resetPasswordForEmail(target.user.email, {
        redirectTo,
      });
    ...
    if (action === "toggle_active") {
      ...
      // Also ban/unban in auth (long ban duration ≈ deactivation)
    ...
    // ---- delete_user (executive only) ----
    if (action === "delete_user") {
      if (callerRole !== "executive") {
        return json({ error: "Only executives can delete users" }, 403);
      }
      const { user_id } = body;
      if (!user_id) return json({ error: "user_id required" }, 400);
